...
int thermodynamics_helium_from_bbn(
  ...
  int num_omegab=0;
  int num_lambda_G_inf=0;

  double * omegab=NULL;
  double * lambda_G_inf=NULL;
  double * YHe=NULL;
  double * ddYHe=NULL;
  double * YHe_at_lambda_G_inf=NULL;
  double * ddYHe_at_lambda_G_inf=NULL;
  ...
  */
  ...
  while (fgets(line,_LINE_LENGTH_MAX_-1,fA) != NULL) {
    ...
    if (left[0] > 39) {
      ...
      /* if the line contains data, we must interpret it. If (num_omegab, num_lambda_G_inf)=(0,0), the current line must contain
         their values. Otherwise, it must contain (omegab, delatN, YHe). */
      if ((num_omegab==0) && (num_lambda_G_inf==0)) {

        /* read (num_omegab, num_lambda_G_inf), infer size of arrays and allocate them */
        class_test(sscanf(line,"%d %d",&num_omegab,&num_lambda_G_inf) != 2,
                   pth->error_message,
                   "could not read value of parameters (num_omegab,num_lambda_G_inf) in file %s\n",ppr->sBBN_file);

        class_alloc(omegab,num_omegab*sizeof(double),pth->error_message);
        class_alloc(lambda_G_inf,num_lambda_G_inf*sizeof(double),pth->error_message);
        class_alloc(YHe,num_omegab*num_lambda_G_inf*sizeof(double),pth->error_message);
        class_alloc(ddYHe,num_omegab*num_lambda_G_inf*sizeof(double),pth->error_message);
        class_alloc(YHe_at_lambda_G_inf,num_omegab*sizeof(double),pth->error_message);
        class_alloc(ddYHe_at_lambda_G_inf,num_omegab*sizeof(double),pth->error_message);
        array_line=0;

      }
      else{

        /* read (omegab, lambda_G_inf, YHe) */
        class_test(sscanf(line,"%lg %lg %lg",&(omegab[array_line%num_omegab]),
                          &(lambda_G_inf[array_line/num_omegab]),
                          &(YHe[array_line])
                          ) != 3,
                   pth->error_message,
                   "could not read value of parameters (omegab,lambda_G_inf,YHe) in file %s\n",ppr->sBBN_file);
        array_line ++;
      }
    }
  }
  ...
  /** - spline in one dimension (along lambda_G_inf) */
  class_call(array_spline_table_lines(lambda_G_inf,
                                      num_lambda_G_inf,
                                      YHe,
                                      num_omegab,
                                      ddYHe,
...
so both omega_b match. */
  omega_b=pba->Omega0_b*pba->h*pba->h/pba->lambda_G_0/pba->lambda_G_0;
  ...
  class_test_except(pba->lambda_G_inf < lambda_G_inf[0],
                    pth->error_message,
                    free(omegab);free(lambda_G_inf);free(YHe);free(ddYHe);free(YHe_at_lambda_G_inf);free(ddYHe_at_lambda_G_inf),
                    "You have asked for an unrealistic small value of lambda_G_inf = %e. The corresponding value of the primordial helium fraction cannot be found in the interpolation table. If you really want this value, you should fix YHe to a given value rather than to BBN",
                    pba->lambda_G_inf);

  class_test_except(pba->lambda_G_inf > lambda_G_inf[num_lambda_G_inf-1],
                    pth->error_message,
                    free(omegab);free(lambda_G_inf);free(YHe);free(ddYHe);free(YHe_at_lambda_G_inf);free(ddYHe_at_lambda_G_inf),
                    "You have asked for an unrealistic high value of lambda_G_inf = %e. The corresponding value of the primordial helium fraction cannot be found in the interpolation table. If you really want this value, you should fix YHe to a given value rather than to BBN",
                    pba->lambda_G_inf);

  /** - interpolate in one dimension (along lambda_G_inf) */
  class_call(array_interpolate_spline(lambda_G_inf,
                                      num_lambda_G_inf,
                                      YHe,
                                      ddYHe,
                                      num_omegab,
  ...
  free(YHe);
  free(ddYHe);
  free(YHe_at_lambda_G_inf);
  free(ddYHe_at_lambda_G_inf);
  free(pvecback);
